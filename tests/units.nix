...
_self: _: {
  ...
  testScript =
    ...
    ''
        ...
        # The cache option lands in nix.conf, URL and key from flake.nix's own
      # nixConfig -- the one copy consumers are told to trust.
      conf = machine.succeed("cat /etc/nix/nix.conf")
      assert "cypherpunktech.cachix.org-1:" in conf and "https://cypherpunktech.cachix.org" in conf, conf
